...
class _StubGeomDeepONetSirenModel:
    ...
    def predict_fields(self, cond_batch: np.ndarray, **_: Any) -> dict[str, np.ndarray]:
        ...
        if self.expected_input_dim is not None and self.last_input_dim != int(self.expected_input_dim):
            raise ValueError(
                f"unexpected cond dim for stub geom_deeponet_siren: got={self.last_input_dim}, "
...
def _table_plus_meta() -> dict[str, Any]:
    ...
    meta[STRUCTURE_ADAPTER_MODE_EFFECTIVE_KEY] = "hybrid_pack_descriptor"
    return meta
...
def test_geom_deeponet_siren_table_plus_descriptor_lane_passes_when_dim_matches(tmp_path: Path) -> None:
    ...
    model = _StubGeomDeepONetSirenModel(expected_input_dim=len(cond_schema.order) + descriptor_dim)
...
def test_geom_deeponet_siren_descriptor_dim_mismatch_is_rejected(tmp_path: Path) -> None:
    geometry_root = _write_parametric_geometry_root(tmp_path / "dataset")
    descriptor_profile = "struct_desc_v1"
    descriptor_dim = _descriptor_dim_for_default_geom(geometry_root, descriptor_profile)
    input_mode_meta = _table_plus_meta()
    engine = InferenceEngine(
        model=_StubGeomDeepONetSirenModel(),
...
def test_geom_deeponet_siren_descriptor_is_cached_per_geom_ref(tmp_path: Path, monkeypatch) -> None:
    geometry_root = _write_parametric_geometry_root(tmp_path / "dataset")
    descriptor_profile = "struct_desc_v1"
    descriptor_dim = _descriptor_dim_for_default_geom(geometry_root, descriptor_profile)
    input_mode_meta = _table_plus_meta()
    engine = InferenceEngine(
        model=_StubGeomDeepONetSirenModel(expected_input_dim=1 + descriptor_dim),
...
def test_geom_deeponet_siren_rejects_table_only(tmp_path: Path) -> None:
    geometry_root = _write_parametric_geometry_root(tmp_path / "dataset")
    table_only_meta = build_input_mode_effective_metadata(
        {
            "runtime": {
    ...
    with pytest.raises(ValueError, match="table_only"):
        InferenceEngine(
            ...
            cond_schema=CondSchema(order=["c0"]),
            axis_schema=AxisSchema(mode="steady"),
            geometry_provider=build_geometry_provider(geometry_root, provider_mode="fixed"),
            output_dir=tmp_path / "infer",
            input_mode="table_only",
            input_mode_meta=table_only_meta,
        )
