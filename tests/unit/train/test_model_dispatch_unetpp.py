...
def _ctx(tmp_path: Path, y_vars: list[str] | None = None) -> TrainDispatchContext:
    ...
    return TrainDispatchContext(
        ...
        model_idx=0,
        model_name="unetpp",
        model_dir=tmp_path / "dispatch_unetpp",
        global_seed=0,
        n_cases=n,
        h=h,
        w=w,
...
def test_unetpp_rejects_non_geom_feature_pack(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    cfg = _valid_cfg(ctx.y_vars)
    cfg["input_features"]["mode"] = "legacy_xy"
    ctx.run_cfg = {"train": {"unetpp": cfg}}
    with pytest.raises(ValueError, match="geom_feature_pack"):
        run_model_train_predict(ctx)


def test_unetpp_rejects_wrong_feature_order(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    cfg = _valid_cfg(ctx.y_vars)
...
def test_unetpp_rejects_non_shared_head(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    cfg = _valid_cfg(ctx.y_vars)
...
def test_unetpp_rejects_non_mainline_selection_mode(tmp_path: Path) -> None:
    ctx = _ctx(tmp_path)
    cfg = _valid_cfg(ctx.y_vars)
