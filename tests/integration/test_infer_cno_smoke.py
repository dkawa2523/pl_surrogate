...
def _cno_dual_mode_cfg(*, run_dir: Path) -> dict:
    return {
        "run_dir": str(run_dir),
        "runtime": {
...
def test_infer_cno_smoke(tmp_path: Path) -> None:
    require_torch_runtime()

    run_dir = tmp_path / "cno_run"
    cfg = _cno_dual_mode_cfg(run_dir=run_dir)
    cfg_path = tmp_path / "cno.yaml"
...
def test_train_checkpoint_load_and_infer_cno_smoke(tmp_path: Path) -> None:
    require_torch_runtime()

    run_dir = tmp_path / "cno_run_ckpt"
    cfg = _cno_dual_mode_cfg(run_dir=run_dir)
    cfg_path = tmp_path / "cno_ckpt.yaml"
    with cfg_path.open("w", encoding="utf-8") as f:
        yaml.safe_dump(cfg, f)
