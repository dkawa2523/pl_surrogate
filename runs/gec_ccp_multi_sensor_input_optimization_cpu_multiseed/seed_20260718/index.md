...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `cmaes` | 4.9573 | 1.5435 | 3.4371 | 1.299 | 3/3 | 53.0 | 1/3 | 56.44 |
| `tpe` | 4.8916 | 1.2376 | 2.8867 | 0.000 | 3/3 | 24.0 | 2/3 | 57.35 |
| `random` | 5.8560 | 3.0233 | 4.0392 | 18.988 | 2/3 | 33.5 | 1/3 | 57.39 |
...
- Existing-FNO marginal CMA search: 56.44 s
- Fixed-budget from-scratch speedup: 4.55x
- Td-wise median-time speedup: 2.12x
- Direct COMSOL oracle-best estimate: 18.191 h
- oracle-bestは最終最良trialを事後に知る下限で、実運用の停止時間ではありません。
- 直接COMSOL時間は既存78ケースのTd別実測平均による推定で、最適候補の新規COMSOL検証は未実施です。

## Interpretation

- clean truth-profile誤差が最小だったのは `tpe` の平均 1.2376%です。
- Randomも3/3ケースを改善しましたが、平均truth-profile誤差は 3.0233%でした。
...
- `tpe` のnoisy-sensor lossは 4.8916%でtrue-input FNOより低い一方、truth/full-field誤差はtrue-inputより高く、ノイズまたはサロゲート誤差の入力補償が含まれます。
...
Total three-measurement execution wall time: 183.84 s
