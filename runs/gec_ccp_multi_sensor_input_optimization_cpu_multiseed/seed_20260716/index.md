...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `cmaes` | 5.0493 | 1.9532 | 4.2176 | 2.460 | 3/3 | 35.0 | 1/3 | 56.34 |
| `tpe` | 5.0423 | 1.8347 | 3.8600 | 1.998 | 3/3 | 32.0 | 2/3 | 56.78 |
| `random` | 5.8167 | 3.6721 | 6.9240 | 17.626 | 1/3 | 86.0 | 0/3 | 56.38 |
...
- Existing-FNO marginal CMA search: 56.34 s
- Fixed-budget from-scratch speedup: 4.55x
- Td-wise median-time speedup: 2.12x
- Direct COMSOL oracle-best estimate: 16.572 h
- oracle-bestは最終最良trialを事後に知る下限で、実運用の停止時間ではありません。
- 直接COMSOL時間は既存78ケースのTd別実測平均による推定で、最適候補の新規COMSOL検証は未実施です。

## Interpretation

- clean truth-profile誤差が最小だったのは `tpe` の平均 1.8347%です。
- Randomも3/3ケースを改善しましたが、平均truth-profile誤差は 3.6721%でした。
...
- `tpe` のnoisy-sensor lossは 5.0423%でtrue-input FNOより低い一方、truth/full-field誤差はtrue-inputより高く、ノイズまたはサロゲート誤差の入力補償が含まれます。
...
Total three-measurement execution wall time: 182.19 s
